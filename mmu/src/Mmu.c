...
#define N_DWORDS BYTES_PER_CACHE_LINE/8
...
uint32_t  readPageTableEntryFromMemory(MmuState* ms, mmu_in *mmu, uint64_t physical_addr);
void      writePageTableEntryToMemory(MmuState* ms, mmu_in *mmu, uint32_t pte, uint64_t physical_addr);
uint8_t   walkPageTables(MmuState* ms, mmu_in *mmu,
...
uint8_t   performMmuProbe(MmuState* ms, mmu_in *mmu, 
			...
			uint8_t byte_mask, uint64_t write_data)
{
    ...
    uint64_t local_line_physical_addr;
    uint8_t probe_success;
    uint32_t pte=0;
    ...
    switch(mmu->mmu_exec_state) {
        ...
        case MMU_INIT:
            ...
            //
                    ...
                    || (!MmuEnabled(ms) && (ms->mmu_is_present && !__CACHEABLE__(ms))  && //Mmu disabled and I/D request
                       ...
                       ))
            {
                ...
                //
                mmu->ptr = ((mmu_command == MMU_READ_LINE) ? (&mmu->line_data[cacheLineOffset(addr)]) 
                        : &mmu->read_data);
                        
                mmu->mmu_exec_state = MMU_PASS_THRU_READ_LINE;
                sysMemBusRequest_push(mmu, (request_type | lock_mask),byte_mask, addr, write_data);
            ...
            else if(asi==ASI_MMU_FLUSH_PROBE)
            {
                if(ms->mmu_is_present) 
                {
                    ...
                    if(request_type==REQUEST_TYPE_READ)
                    {
                        ...
                        mmu->mmu_probe_state = MMU_PROBE_IN;
                        
                        probe_success = performMmuProbe(ms, mmu, addr, asi, request_type, &pte);
						assert(probe_success != 3);
                        ...
                        if(probe_success == 1)
                        {
                            ...
                            if(getBit32(addr,2)==0) 
                                mmu->read_data = mmu->read_data << 32;

                            // If an error was caused, the FSR/FAR are modified inside
                            // the performMmuProbe() routine. 
                            mmu->mmu_exec_state = MMU_EXIT;
                            return;
                        }
                        else if(probe_success == 2)
                        {
                            mmu->mmu_exec_state = MMU_PROBE;                            
                            return;
                        } else {
                            mmu->mmu_exec_state = MMU_EXIT; // Can happen
                            return;
                        }
                    }
                    //Flush operation
                    else if(request_type==REQUEST_TYPE_WRITE)
                    {
                        ...
                        mmu->read_data=0;	mmu->cacheable=0;
                ...
                {
                        mmu->read_data=0;	mmu->cacheable=0; mmu->mae = 0;
            ...
            else if(asi==ASI_MMU_REGISTER)
            {
                if(ms->mmu_is_present)
                {
                    ...
                    else if(request_type==REQUEST_TYPE_WRITE)
                    {
                        ...
                        //send response to cpu
                        mmu->mae=0; mmu->read_data=0; 	mmu->cacheable=0;
            ...
            {
                physical_addr=0;
                mmu->translation_success=0;

                //perform translation of virtual
                //address to physical address.
                mmu->mmu_trans_phy_state = TRANS_PHYS_ENTRY;
                case6 = 1;
                mmu->translation_success = 
                    translateToPhysicalAddress(ms, mmu,
                            asi, addr, request_type, 
                            &physical_addr, 
                            &mmu->cacheable, 
                            &mmu->acc,
                            &mmu->mmu_fsr);
                if (mmu->translation_success == 2) {
                    mmu->mmu_exec_state = MMU_PHY_TRANSLATE;
                    return;
        ...
        case MMU_PHY_TRANSLATE:
        
            if(mmu->mmu_exec_state == MMU_PHY_TRANSLATE) {
                mmu->translation_success = 
                    translateToPhysicalAddress(ms, mmu,
                            asi, addr, request_type, 
                            &physical_addr, 
                ...
                if ((mmu->mmu_exec_state == MMU_INIT) && (case6 == 0)) return;
            ...
            if(mmu->translation_success)
            {

                int I;

                mmu->line_physical_addr = physical_addr & 0xffffffffffffffc0;
                ...
                if(mmu_command == MMU_READ_LINE)
                {
                    if(mmu->cacheable)
                    {
                        ...
                        // fetch the line.
                        mmu->ptr = &mmu->line_data[0];                        
                        sysMemBusRequest_push(mmu, (REQUEST_TYPE_READ | lock_mask),byte_mask, mmu->line_physical_addr, write_data);
                        mmu->read_line_count = 1;
                        mmu->mmu_exec_state = MMU_READ_LINE_STATE;
                        return;
                        
                        // Now the coming loop has to go in the case for MMU_READ_LINE_STATE
                        ...
                        // not cacheable... do not read the whole line...
                    {
                        ...
                        mmu->ptr = &mmu->line_data[I];
                        sysMemBusRequest_push(mmu, (REQUEST_TYPE_READ | lock_mask),byte_mask, mmu->line_physical_addr + (8*I), write_data);
                        mmu->mmu_exec_state = MMU_NOT_CACHEABLE_READ_WORD;                        
                        return;
                    }
                }
                else 
                {
                    mmu->ptr = &mmu->read_data;
                    sysMemBusRequest_push(mmu, (request_type | lock_mask),byte_mask, physical_addr, write_data);
                    mmu->mmu_exec_state = MMU_NOT_READ_LINE;
                    return;
        ...
        case MMU_READ_LINE_STATE:
        
            mmu->ptr = &mmu->line_data[mmu->read_line_count];
            local_line_physical_addr = mmu->line_physical_addr + 8*mmu->read_line_count;                        
            sysMemBusRequest_push(mmu, (REQUEST_TYPE_READ | lock_mask),byte_mask, local_line_physical_addr, write_data);
            if (mmu->read_line_count < N_DWORDS) return; 

        case MMU_NOT_CACHEABLE_READ_WORD:
        case MMU_NOT_READ_LINE:
            //send the response back to the cache
            //that created this request
            // Move these to end of MMU_NOT_READ_LINE, MMU_NOT_CACHEABLE_READ_WORD, MMU_READ_LINE_STATE
            mmu->mae=0;
            mmu->mmu_fsr=0;
        ...
        case MMU_PASS_THRU_READ_LINE:
            //send the response back to the cache
            //that created this request
            mmu->mae=0;
            mmu->cacheable  = __CACHEABLE__(ms)  && !ASI_MMU_PASS_THROUGH(asi);
            ms->Num_Mmu_bypass_accesses +=1;
            return;
                
        case MMU_PROBE:
            mmu->mmu_probe_state = MMU_PROBE_OUT;
            probe_success = performMmuProbe(ms, mmu, addr, asi, request_type, &pte);
            assert(probe_success != 3);
		...
		uint32_t virt_addr, uint8_t asi, uint8_t request_type, uint32_t* PTE)
{
	...
	*PTE=0;
    ...
    switch(mmu->mmu_probe_state) {
        
        case MMU_PROBE_IN:
            if (TLB_ENABLED && isTlbNewHit(ms, va_tag, &pte, &pte_level)) {
                *PTE = pte;
                return 1;
            }
            mmu->mmu_walk_pt_state = WALK_PT_L0;
            
        case MMU_PROBE_OUT:
            pte_found = walkPageTables(ms, mmu, virt_addr, &pte, &pte_level, &phy_addr_of_pte);

            //check if PTE is valid, else update FSR and FAR registers
            switch(pte_found) {
                case 0:

                    checkPageFaults(ms, pte, pte_level, asi, virt_addr, request_type, &fsr_to_be_updated_after_probe); 
                    *PTE=0;
                    return 0;
                ...
                case 2:
                    mmu->mmu_probe_state = MMU_PROBE_OUT;// not redundant
                    return 2;
            }
		...
		uint32_t* mmu_fsr_to_be_returned)
{
    ...
    uint8_t	   pte_found;		//whether PTE was found after walking page tables in memory
    uint32_t   pte;		//the value of PTE read from TLB or fetched from page tables in memory
    uint8_t	   pte_level;		//the level of PTE read from TLB or fetched from memory
    uint64_t   phy_addr_of_pte;   //the physical address where this PTE was found in memory
     //virtual addr tag
    uint8_t    access_allowed;
    uint8_t    pte_found_in_mem;
    uint32_t   pte_in_mem;
    uint8_t     pte_level_in_mem;
    uint64_t    phy_addr_of_pte_in_mem;

    switch(mmu->mmu_trans_phy_state) {
        
        case TRANS_PHYS_ENTRY:        
        
            if(!MmuEnabled(ms) || !ms->mmu_is_present)
            {
                // If mmu is absent, then it is always cacheable
                // because what else can you do?
                *cacheable = !ms->mmu_is_present || __CACHEABLE__(ms);
                *acc = 3;
            ...
            mmu->TLB_hit=0;		//whether PTE was found in TLB
            pte_found=0;		//whether PTE was found after walking page tables in memory
            pte=0;		//the value of PTE read from TLB or fetched from page tables in memory
            pte_level=0;		//the level of PTE read from TLB or fetched from memory
            phy_addr_of_pte=0;   //the physical address where this PTE was found in memory
            mmu->va_tag=getSlice32(virt_addr, 31,12); //virtual addr tag
            ...
            *mmu_fsr_to_be_returned = 0;
            ...
            if(TLB_ENABLED)
            {
                ...
                mmu->TLB_hit = isTlbNewHit(ms, mmu->va_tag, &pte, &pte_level);
            ...
            if(!TLB_ENABLED || !mmu->TLB_hit)
            {
                ...
                pte_found = walkPageTables(ms, mmu, virt_addr, &pte, &pte_level, &phy_addr_of_pte);
                assert(pte_found == 2);
                mmu->mmu_trans_phy_state = TRANS_PHYS_WALKING_1;
                return 2;

            }

        case TRANS_PHYS_WALKING_1:

            // We could be falling through also. So we check
            if (mmu->mmu_trans_phy_state == TRANS_PHYS_WALKING_1) {
            ...
            access_allowed = checkPageFaults(ms, pte, pte_level, asi, virt_addr, request_type, mmu_fsr_to_be_returned);
            if(!pte_found || !access_allowed) {
                mmu->mmu_trans_phy_state = TRANS_PHYS_EXIT;
                return 0;
            }
            ...
            if(TLB_ENABLED && !mmu->TLB_hit)
            {
                ...
                writeTlbNewEntry(ms, virt_addr, mmu->va_tag, pte, pte_level);
                mmu->TLB_hit=1;
            }
            ...
            *acc  = getSlice32(pte,4,2);
            ...
            if(request_type==REQUEST_TYPE_WRITE) mmu->updated_pte=setBit32(mmu->updated_pte,6,0x1);
            //set the R (read) bit when the page is accessed
            mmu->updated_pte=setBit32(mmu->updated_pte,5,0x1);

            if(!(mmu->updated_pte==pte))
            {
                ...
                pte_in_mem=0;
                pte_level_in_mem=0;
                phy_addr_of_pte_in_mem=0;
                mmu->mmu_walk_pt_state = WALK_PT_L0;
                pte_found_in_mem = walkPageTables(ms,  mmu,
                        virt_addr, &pte_in_mem, &pte_level_in_mem, 
                        &phy_addr_of_pte_in_mem);
                assert(pte_found_in_mem == 2);
                mmu->mmu_trans_phy_state = TRANS_PHYS_WALKING_2;
                return 2;                

            }
            return 1;

        case TRANS_PHYS_WALKING_2:
            pte_found_in_mem = walkPageTables(ms,  mmu, virt_addr, &pte_in_mem, &pte_level_in_mem, &phy_addr_of_pte_in_mem);
            if (pte_found_in_mem == 2)
                return 2;
            
            // This whole if can go to TRANS_PHYS_WALKING_2
            if(!pte_found_in_mem)
            {
#ifdef SW
            ...
            else
            {
                ...
                mmu->mmu_trans_phy_state = TRANS_PHYS_EXIT;
                mmu->mmu_read_pt_state = PTE_READ_WRITE_IN;
                writePageTableEntryToMemory(ms, mmu, mmu->updated_pte, phy_addr_of_pte);
                assert(mmu->mmu_read_pt_state == PTE_READ_WRITE_OUT);
                return 2;
            }
            
        case TRANS_PHYS_EXIT:
            //also update the value of PTE in TLB
            if(TLB_ENABLED)
            {
                writeTlbNewEntry(ms, virt_addr, mmu->va_tag, mmu->updated_pte, pte_level);
            }
...
uint32_t readPageTableEntryFromMemory(MmuState* ms, mmu_in *mmu, uint64_t physical_addr)
{
	...
	switch(mmu->mmu_read_pt_state) {
		
		case PTE_READ_WRITE_IN:
			lock_mask = (ms->lock_flag << 6);
			mmu->ptr = &mmu->read_data;
			sysMemBusRequest_push(mmu, (request_type | lock_mask),byte_mask, physical_addr, data64);
			mmu->mmu_read_pt_state = PTE_READ_WRITE_OUT;
			return 0;
			break;
			
		case PTE_READ_WRITE_OUT:
			data64 = mmu->read_data;
...
void writePageTableEntryToMemory(MmuState* ms, mmu_in *mmu, uint32_t pte, uint64_t physical_addr)
{
    ...
    assert(mmu->mmu_read_pt_state == PTE_READ_WRITE_IN);
		...
		uint32_t virt_addr, uint32_t* pte, uint8_t* pte_level, uint64_t* phy_addr_of_pte)
{
	...
	switch(mmu->mmu_walk_pt_state) {
		
		case WALK_PT_L0:
			//read Level 0 PTD/PTE entry from context table
			PTE_LEVEL =0;
			context_table_index= ms->MmuContextRegister;	// Index into context table
			phy_addr = getPhyAddrFromPTD(ms->MmuContextTablePointerRegister, context_table_index);
			mmu->mmu_read_pt_state = PTE_READ_WRITE_IN;
			mmu->mmu_walk_pt_state = WALK_PT_L1;
			PTE=readPageTableEntryFromMemory(ms, mmu, phy_addr);
			assert(PTE==0);
			pte_found = 2;
			break;
		...
		case WALK_PT_L1:
			...
			if(getSlice32(PTE,1,0)==0x1) //This is a PTD. Fetch entry from Level1 Table
			{
				...
				mmu->mmu_walk_pt_state = WALK_PT_L2;
				mmu->mmu_read_pt_state = PTE_READ_WRITE_IN;
				PTE=readPageTableEntryFromMemory(ms, mmu, phy_addr);
				assert(PTE==0);
				pte_found = 2;				
		...
		case WALK_PT_L2:
			...
			if(getSlice32(PTE,1,0)==0x1) //This is a PTD. Fetch entry from Level1 Table
			{
				...
				mmu->mmu_walk_pt_state = WALK_PT_L3;
				mmu->mmu_read_pt_state = PTE_READ_WRITE_IN;
				PTE=readPageTableEntryFromMemory(ms, mmu, phy_addr);
				assert(PTE==0);
				pte_found = 2;				
		...
		case WALK_PT_L3:
			...
			if(getSlice32(PTE,1,0)==0x1) //This is a PTD. Fetch entry from Level1 Table
			{
				...
				mmu->mmu_walk_pt_state = WALK_PT_LRET;
				mmu->mmu_read_pt_state = PTE_READ_WRITE_IN;
				PTE=readPageTableEntryFromMemory(ms, mmu, phy_addr);
				assert(PTE==0);
				pte_found = 2;				
		...
		default:
			assert(0); // This is no place to fall.
			
	}
	//Check if page walk ended.
	//We have either found a valid PTE or not
	if (PTE != 0) // Tricky business. We are assuming that PTE == 0 is an invalid entry
		if( getSlice32(PTE,1,0)==0x02) 
				pte_found=1;//We have found a valid PTE
...
void printMmuStatistics(MmuState* ms)
{
	...
	if(ms->mmu_is_present)
	{
        fprintf(stderr,"\nMmu statistics for core=%d, MMU ENABLED = %d", 
                ms->core_id, MmuEnabled(ms));
